(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('sptMonitorApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-sptMonitorApp-error');
                var entityKey = httpResponse.headers('X-sptMonitorApp-params');
                if (errorHeader) {
